...
public class HeroDigger extends HerobrineEvent{
    ...
    public void trigger(){
        ...
        while (!player.getWorld().getName().equals(world.getName()) && breakcheck < 100) {
            index = (int) (Math.random() * players.size());
            this.player = players.get(index);
            breakcheck++;
        }
        if (breakcheck == 100 && !player.getWorld().getName().equals(world.getName())) {
            return;
        }
        ...
        this.herobrine = new HerobrineCharacter(this.player.getWorld(), x, y, z, (float) d3, (float) d4, new Item(Item.Type.DiamondSpade, 1));
